...
import { Bell, BookOpen, CalendarDays, CreditCard, FileText, Flag, Home, LayoutDashboard, Menu, MessageCircle, Newspaper, ServerCog, Settings, Star, Trash2, UserRound, Users, X } from 'lucide-react'
...
export default function AgentWorkspaceShell({ children, badges }: { children: React.ReactNode; badges: Partial<Record<string, number>> }) {
  ...
  return <div className={styles.root}>
    <aside className={styles.sidebar}><Link href="/agent" className={styles.brand} aria-label="LIVEINBLACK"><Image src="/branding/liveinblack-logo-horizontal.png" alt="LIVEINBLACK" width={614} height={217} className={styles.brandLogo} priority /></Link><nav className={styles.nav} aria-label="Administration">{navigation}</nav><div className={styles.sidebarFoot}>{session?.user ? <div className={styles.sidebarAccount}><AccountMenu user={session.user} menuAlign="left" menuDirection="up" dashboardMode /></div> : null}<Link href="/home" className={styles.publicLink}><Globe size={17} /><span>Accueil</span></Link></div></aside>
